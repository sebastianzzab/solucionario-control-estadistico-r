...
Este solucionario ha sido elaborado con base en los contenidos presentados en el libro *Control Estadístico de la Calidad*, desarrollado por el profesor **Oswaldo Bello** como material de apoyo para la asignatura del mismo nombre en la Universidad de Oriente, Núcleo Nueva Esparta.

Estos ejercicios aplican los conceptos del [libro de Control Estadístico de la Calidad](https://oswaldobelloc.github.io/libro_cec/) del profesor Oswaldo Bello, especialmente los relacionados con los diagramas de control para atributos.
...
El diagrama $np$ se utiliza para monitorear el número de unidades defectuosas en un proceso de producción. Se aplica específicamente cuando el tamaño de la muestra es constante, permitiendo a los analistas observar la variabilidad en el número de defectos a lo largo del tiempo.
...
#| fig-cap: "Diagrama $np$ para el número de componentes k12 defectuosos"
...
Al eliminar estas observaciones la nueva estimación de $p$ sería $`r formato(p_est)`$ y los limites de control superior e inferior serían $`r formato(lsc_tmp)`$ y $`r formato(lic_tmp)`$, respectivamente.
...
Se observa en la @fig-ejemplo-dcnp2-highcharter que eliminando las observaciones que estuvieron fuera de control, el proceso entra en estado bajo control. Por lo que, se concluye que la estimación de $p$ ($\bar{p}=`r formato(p_est)`$) es correcta para la supervisión futura de la producción de de componentes k12.
...
El diagrama $p$ muestra las variaciones en la proporción de artículos defectuosos por muestra o subgrupo. Es ampliamente usada para evaluar el desempeño de una parte o de todo un proceso, tomando en cuenta su variabilidad con el propósito de detectar causas o cambios especiales en el proceso.
...
Se requieren los parametros para construir el diagrama de control. Nuevamente como no se tiene conocimiento sobre los parametros del proceso se tienen que estimar. Nuevamente se estima $p$ a través de $\hat{p}$ con la @eq-pestnp. Cuyo resultado con R es el siguiente:
...
highchart() |>
  ...
  hc_yAxis(
    title = list(
      useHTML = TRUE,
      text = "<b><i>Fracción de de piezas con rebaba</i></b><br>"
    )
  ) |>
...
#| tbl-cap:  "Cantidad de piezas de válvulas rechazadas por cada lote"
#| tbl-align: left
...
Al no tener el mismo tamaño de muestra entre lotes, laa estimación de $p$ se obtiene mediante la siguiente expresión:
...
highchart() |>
  ...
  hc_yAxis(
    title = list(
      useHTML = TRUE,
      text = "<b><i>Fracción de piezas defectuosas (p<sub>i</sub>)</i></b><br>"
    ),
    min = 0,
    max = 0.1
  ) |>
...
Como se puede observar en la @fig-dcp-tmv-highcharter el proceso funciona bajo control. Por lo que a la empresa del ramo metalmecánico que fabrican válvulas las estimaciones realizadas les serviran de base para proximas inspecciones de los lotes de piezas en el proceso de fundición, para evaluar si el proceso sigue bajo control.
:::
::::

## Diagrama $c$

El diagrama $c$ analiza la variabilidad del número de defectos en un proceso. Se aplica específicamente cuando el tamaño de la muestra o subgrupo se mantiene constante. permite visualizar y monitorear el número de defectos encontrados al inspeccionar unidades de producto o servicio, ayudando a identificar tendencias o variaciones en el proceso de producción.
...
El diagrama $u$ permite monitorear el número promedio de defectos por unidad en un proceso de producción es especialmente útil cuando el tamaño de las muestras varía, lo que permite un análisis más flexible y adaptado a diferentes situaciones de producción. Permite a las organizaciones evaluar la calidad de sus productos al registrar el número promedio de defectos por unidad, incluso cuando el tamaño de las muestras no es constante.
...
lic <-  max(u_est - 3 * desviacion, 0)
lc <- u_est
lsc <- u_est + 3 * desviacion
