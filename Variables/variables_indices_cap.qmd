---
title: "Diagramas de Control para Variables y Análisis de Indices de Capacidad de Procesos Aplicados con ${\\color{Blue} \\textbf{\\textsf{R}}}$"
---
...
::: {#exm-dcxr .Example}
### Aplicación del diagrama de control para la Media($\bar{X}$) y el Rango($R$) {#sec-ejemplo-diagrama-xr}

<br/>Se van a establecer gráficas de control para $\bar{X}$ y $R$ en determinada dimensión, en milímetros, de una parte. Se reunieron datos en tamaños de subgrupo igual a 6, y se citan a continuación en la @tbl-datos-peso-promedio-rango-parte. Determine la línea central y los límites de control tentativos. Suponga que hay causas asignables, y revise la línea central y los límites.Este ejemplo fue tomado de @besterfield1995control.

```{r}
#| label: tbl-datos-peso-promedio-rango-parte
#| tbl-cap:  "Mediciones del peso promedio y el rango en milímetros de una parte"
#| tbl-colwidths: [20,50]
...
#| fig-cap: "Diagramas $R$ para el diámetro interior (mm) de los anillos en la fase I con el paquete `highcharter`"
...
Se puede observar en la @fig-R-highcharter el rango asociado al subgrupo 18 se encuentra fuera de los limites de control, por lo que suponiendo causas asignables el proceso esta fuera de control estadístico. Por lo que se tiene que recalcular los limites de control y construir nuevamente el diagrama $R$.

```{r}
#| label: fig-xbarra-highcharter
#| fig-cap: "Diagramas $\\bar{x}$ para el diámetro interior (mm) de los anillos en la fase I con el paquete `highcharter`"
...
# Verifica si tus 'R' (datos$rango después de la eliminación) tienen variabilidad.
#print(paste("R_revisado después de la eliminación:", paste(R, collapse = ", ")))
#print(paste("Media de R_revisado:", R_barra))
#print(paste("Desviación estándar estimada del proceso (sd_estimado_proceso):", sd_estimado_proceso))


# Realizar el análisis de capacidad del proceso
# Pasamos la desviación estándar estimada al objeto qcc
# NOTA: qcc(data, type = "xbar", ...) espera 'data' sean los *datos crudos* para calcular medias y std.dev.
# O si le pasas las medias de los subgrupos (como estás haciendo con X_bar),
# DEBES proporcionar una estimación de la desviación estándar del proceso ('std.dev')
# o el rango promedio ('R.bar') si quieres que la estime de ahí.
...
## Diagrama de control para la Media($\bar{X}$) y la Desviación Estándar($S$)
...
:::: {.callout-tip icon="false" appearance="simple"}
::: {#exm-dcxs .Example}
### Aplicación del diagrama de control para la Media($\bar{X}$) y la Desviación Estándar($S$) {#sec-ejemplo-diagrama-xs}

<br/>Se van a establecer gráficas de control para la dureza Brinell de acero endurecido para herramientas, en kilogramos por milímetro cuadrado. A continuación se muestran los datos para tamaños de subgrupo igual a 8. Determine la línea central y los límites de control tentativos para las gráficas $\bar{x}$ y $s$. Suponga que los puntos fuera de control tienen causas asignables, y calcule los límites y línea central revisados. Este ejemplo fue tomado de @besterfield1995control.

```{r}
#| label: tbl-datos-dureza_Brinell
#| tbl-cap:  "Mediciones para la dureza Brinell de acero endurecido para herramientas, en kilogramos por milímetro cuadrado"
...
## Diagrama de control para la Media($\bar{X}$) y la Desviación Estándar($S$) con tamaño de muestra variable

:::: {.callout-tip icon="false" appearance="simple"}
::: {#exm-dxstmv .Example}
## Aplicación de los Diagramas $\bar{x}$ y $S$ con tamaño de muestra variable. {#sec-ejemplo-diagrama-xstmv}

<br/>Una empresa de producción de alimentos se encuentra interesada en supervisar la concentración de un conservante en los productos que ofrece, para ello toman entre 10 y 15 muestras de 20 lotes de producción distintos, midiendo la concentración del conservante en su respectiva unidad.

La @tbl-datos-ejemplo-dxstmv muestra las medias y las desviaciones que presentó cada lote inspeccionado, junto con su la cantidad de muestras que se tomó en el lote correspondiente. Este ejemplo fue tomado de @rendon2013control.
...
La representación gráfica de este diagrama se observa en la @fig-dcxbarraStmv-qcc:
...
#     color = "red", dashStyle = "Dash", name = "<b><i>LCS</b></i>",
#     step = "hv", marker = list(enabled = FALSE),
#     tooltip = list(valueDecimals = 4) # Ajusta los decimales según la precisión de tus datos
#   ) |>
#   hc_add_series(
#     datos_xstmv,
...
#     datos_xstmv,
#     type = "line", color = "black",
#     hcaes(x = obs, y = des), # 'des' es la columna de desviación estándar/rango
...
#     title = list(
#       useHTML = TRUE,
#       text = "<b><i>Desviación estándar (S<sub>i</sub>)</b></i><br>"
#     )
#   ) |>
#   hc_tooltip(crosshairs = TRUE, shared = TRUE) |>
#   hc_plotOptions(series = list(animation = FALSE)) |>
#   hc_title(text = "Diagrama S inicial") # Título para el sub-gráfico
```


Se observa que tanto el diagrama de control de la **desviación estándar ($S$)** como el diagrama de control de la **media ($\bar{X}$)**, como se muestra en la Figura @fig-dcxbarraStmv-qcc, tienen **todos sus puntos dentro de los límites de control**.
...
Los límites de tolerancia natural fueron los establecidos anteriormente, siendo estos $`r formato(lci)`$ y $`r formato(lcs)`$. 
...
:::: {.callout-tip icon="false" appearance="simple"}
::: {#exm-dcmr .Example}
### Aplicación del diagrama de control $MR$ {#sec-ejemplo-diagrama-mr}

<br/>En el departamento de sistemas se lleva un registro del tiempo de respuesta a solicitudes de servicio de clientes internos. Los últimos datos en horas y en el orden de ocurrencia se muestran en la @tbl-datos-tiempo_respuesta_solicitudes. Obtenga los límites de control para una carta de control de individuales e interprételos. Este ejemplo fue tomado de @gutierrez2009control.
...
Uno de los requisitos para poder hacer un diagrama de control para muestras individuales es que las mismas se distribuyan de manera normal. Por lo que para probar esto se utilizara la prueba de Kolmogorov. La misma se hace en R de la siguiente forma y se tienen los siguientes resultados:

```{r}
# --- Apply the One-Sample Kolmogorov-Smirnov Test for Normality ---

# The ks.test function requires the mean and standard deviation of the
# hypothesized normal distribution. We'll use the sample mean and SD
# as estimates for the population parameters.
sample_mean <- mean(individual_times$Tiempo_Respuesta_Horas)
sample_sd <- sd(individual_times$Tiempo_Respuesta_Horas)

# Perform the Kolmogorov-Smirnov test
# The 'pnorm' function specifies that we are testing against a normal distribution.
ks_test_result <- ks.test(
  x = individual_times$Tiempo_Respuesta_Horas,
  y = "pnorm",
  mean = sample_mean,
  sd = sample_sd
)
...
cat(paste0("Estadístico D de Kolmogorov-Smirnov: ", round(ks_test_result$statistic, 4), "\n"))
cat(paste0("Valor p (p-value): ", round(ks_test_result$p.value, 4), "\n"))

# Common significance level (alpha) is 0.05
alpha <- 0.05

if (ks_test_result$p.value < alpha) {
  cat(paste0("Dado que el valor p (", round(ks_test_result$p.value, 4), ") es menor que el nivel de significancia (", alpha, "),\n"))
  cat("rechazamos la hipótesis nula.\n")
  cat("Esto sugiere que los datos NO provienen de una distribución normal.\n")
} else {
  cat(paste0("Dado que el valor p (", round(ks_test_result$p.value, 4), ") es mayor que el nivel de significancia (", alpha, "),\n"))
  cat("no rechazamos la hipótesis nula.\n")
  cat("Esto sugiere que los datos SÍ podrían provenir de una distribución normal.\n")
}
```

Una vez determinado la existencia de no normalidad de los datos. Se procedera a normalizarlos para fines del analisis a traves de la utilizacion de puntajes Z. Lo cual se hace en R de la siguiente forma:

```{r}
# --- Calcular Z-scores para normalizar los datos ---
# Calcular la media y desviación estándar de todos los datos individuales
mu_x <- mean(individual_times$Tiempo_Respuesta_Horas)
sigma_x <- sd(individual_times$Tiempo_Respuesta_Horas) # Desviación estándar de la población de la Fase I

# Calcular Z-scores: Z = (X - mu) / sigma
individual_times <- individual_times |>
  dplyr::mutate(
    Z_score = (Tiempo_Respuesta_Horas - mu_x) / sigma_x,
    MR = c(NA, abs(diff(Z_score)))
  )

# --- Definir límites de control para el gráfico de Z-scores ---
# La línea central siempre es 0
lc_z <- 0

# Los límites de control siempre están en ± k, donde k es típicamente 3 (para 3-sigma)
k_value <- 3
lcs_z <- k_value
lci_z <- -k_value
...
Una vez obtenido los puntajes z y los limites de control, ya es posible graficar los diagramas de control. Los cuales se pueden observar a continuacion:

```{r}
#| label: fig-MRinicial-highcharter
#| fig-cap: "Diagrama $\\textit{MR}$ inicial para el tiempo de respuesta a solicitudes de servicio de clientes internos"

highchart() |>
  hc_add_series(
    data = individual_times, 
    hcaes(x = Observacion, low = lc_z, high = lcs_z),
    step = "hv", marker = list(enabled = FALSE, visible = FALSE),
    type = "arearange", fillOpacity = 0.2, 
    name = "", color = "green",
    showInLegend = FALSE, tooltip = list(pointFormat = "{NULL}")
  ...
  hc_add_series(
    data = individual_times, 
    hcaes(x = Observacion, low = lci_z, high = lc_z),
    step = "hv", marker = list(enabled = FALSE, visible = FALSE),
    type = "arearange", fillOpacity = 0.2, 
    name = "", color = "blue",
    showInLegend = FALSE, tooltip = list(pointFormat = "{NULL}")
  ...
  hc_add_series(
    data = individual_times, 
    type = "line", hcaes(x = Observacion, y = lcs_z), useHTML = TRUE,
    color = "red", dashStyle = "Dash", name = "<b><i>LCS</b></i>",
    step = "hv", marker = list(enabled = FALSE),
    tooltip = list(valueDecimals = 2)
  ) |>
  hc_add_series(
    data = individual_times,
    type = "line", hcaes(x = Observacion, y = lc_z), useHTML = TRUE,
    color = "blue", name = "<b><i>LC</b></i>",
    step = "hv", marker = list(enabled = FALSE),
    tooltip = list(valueDecimals = 2)
  ) |>
  hc_add_series(
    data = individual_times, 
    type = "line", hcaes(x = Observacion, y = lci_z), useHTML = TRUE,
    color = "red", dashStyle = "Dash", name = "<b><i>LCI</b></i>",
    step = "hv", marker = list(enabled = FALSE),
    tooltip = list(valueDecimals = 2)
  ) |>
...
#| fig-cap: "Diagrama $x$ incial para el tiempo de respuesta a solicitudes de servicio de clientes internos"
